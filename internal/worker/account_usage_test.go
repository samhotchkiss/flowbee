...
// TestAgentHitLimit proves the binary rate-limit BACKSTOP fires on the hard-limit
// phrases (preserved so a provider that 429s before the estimate is still gated).
func TestAgentHitLimit(t *testing.T) {
	...
	for _, s := range hits {
		if !agentHitLimit(s) {
			t.Fatalf("agentHitLimit should fire on %q", s)
		}
	}
	if agentHitLimit("ran fine, used 500 tokens") {
		t.Fatal("agentHitLimit must not fire on benign output")
	}
}
