...
func TestAdoptIssueReadoptsAfterCancel(t *testing.T) {
	...
	id1, err := st.AdoptIssueAsBuild(ctx, "flowbee", 47, "glossary", "body", "base", 0, now)
	if err != nil || id1 == "" {
		t.Fatalf("first adopt: id=%q err=%v", id1, err)
	}
	// a live job tracks the issue -> a re-adopt is a no-op (no duplicate).
	if id2, _ := st.AdoptIssueAsBuild(ctx, "flowbee", 47, "glossary", "body", "base", 0, now); id2 != "" {
	...
	id3, err := st.AdoptIssueAsBuild(ctx, "flowbee", 47, "glossary", "body", "base", 0, now)
