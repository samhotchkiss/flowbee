...
// SeedParams describes a hand-seeded ready build job (the M1 manual seed).
...
// SeedJob inserts a `ready` job and its job_created event in one transaction
// (append + projection are atomic). Returns the seeded job.
func (s *Store) SeedJob(ctx context.Context, p SeedParams) (job.Job, error) {
	...
	err := s.tx(ctx, func(tx *sql.Tx) error {
		_, err := tx.ExecContext(ctx, `
			...
			VALUES (?, ?, ?, ?, 'ready', ?, ?, ?, 0, 0, 5, 0, 3, 0)`,
			p.ID, string(p.Kind), p.Flow, p.Stage, string(p.Role), p.BaseSHA, p.Priority)
		if err != nil {
...
// PickReadyJob returns the id of the highest-priority, oldest `ready` job, or
// "" if none. Selection is advisory; the atomic claim's WHERE state='ready' is
// the correctness guarantee.
func (s *Store) PickReadyJob(ctx context.Context) (string, error) {
	var id string
	err := s.DB.QueryRowContext(ctx, `
		SELECT id FROM jobs WHERE state='ready'
		 ORDER BY priority DESC, enqueued_at ASC, id ASC LIMIT 1`).Scan(&id)
	if errors.Is(err, sql.ErrNoRows) {
		return "", nil
	}
	if err != nil {
...
const jobSelect = `
	SELECT id, kind, flow, stage, state, role,
	       ...
	       priority, COALESCE(lease_id,''), lease_epoch,
...
func scanJob(row rowScanner) (job.Job, error) {
	...
	err := row.Scan(&j.ID, &kind, &j.Flow, &j.Stage, (*string)(&j.State), &role,
		&j.BaseSHA, &j.HeadSHA, &j.Priority, &j.LeaseID, &j.LeaseEpoch,
