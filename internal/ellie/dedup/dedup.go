...
func Run(ctx context.Context, ledger maintenance.CompletedLedger, opts Options) (maintenance.GateStats, error) {
	if opts.CandidateSource == nil {
		return maintenance.GateStats{}, errors.New("dedup sweep candidate source is required")
	}
	return maintenance.RunLLMSweep(ctx, ledger, maintenance.RunOptions{
		StoreID:         opts.StoreID,
		SweepType:       maintenance.SweepDedup,
		CandidateSource: requireDedupCandidates(opts.CandidateSource),
		Judge:           opts.Judge,
...
func requireDedupCandidates(source maintenance.CandidateSource) maintenance.CandidateSource {
	return func(ctx context.Context) ([]maintenance.Candidate, error) {
		candidates, err := source(ctx)
		if err != nil {
			return nil, err
		}
		if err := maintenance.RequireCandidateKinds(maintenance.SweepDedup, candidates, maintenance.CandidatePair, maintenance.CandidateCluster); err != nil {
			return nil, err
		}
...
func GateCandidates(ctx context.Context, ledger maintenance.CompletedLedger, storeID string, candidates []maintenance.Candidate) ([]maintenance.Candidate, maintenance.GateStats, error) {
	...
	return maintenance.FilterEligible(ctx, ledger, storeID, maintenance.SweepDedup, candidates)
}
